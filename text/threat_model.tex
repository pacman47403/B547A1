...
We identified 8 tampering threats and accepted 4 into the risk tables. \pat{need to update these summaries after our threat modeling is complete} Threat ID 004, ``Malware Infested Update'' is a significant threat and represents a category of several different threats which are intended to compromise the USB device by delivering malware which can alter the reset counter, keys, and data within the USB device. \pat{should we do an attack tree for this and associated threats?} Our tampering threats and potential mitigations are depicted in table~\ref{tab:tampersum}.
