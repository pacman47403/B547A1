...
We identified two elevation of privilege threats, one associated with fuzzing
the USB to discovery entry points and another attack that interrupts the device
reset procedure which retains the old password during a device reset.  Elevation
...
Following the OWASP risk mitigation methodology, we accepted XX \pat{put final
number in here} risks.  The full risks, risk tables and methodologies are
summarized in \nameref{ch:a4}. We identified 8 category 2 (\emph{\textbf{high risk}})
