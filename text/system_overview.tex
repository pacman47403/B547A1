...
shared with the host OS.\sidenote{The device unique identifier and private key are contained within the silicon of the device and can only be accesed through an authenticated secure session application hosted on the USB drive.}  The USB also provides cryptographic functions via a cryptochip inside the USB.  The USB also provides a unique identifier which is contained within its embedded circuitry.
